...
export function Home() {
  ...
  return (
    <div className="space-y-5 px-5 pt-6">
      ...
      {/* Weekly streak status card */}
      <div
        className={`rounded-3xl p-5 text-white shadow-lg ${
          alive ? 'bg-gradient-to-br from-orange-400 to-rose-500' : 'bg-gradient-to-br from-slate-400 to-slate-600'
        }`}
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-4xl font-extrabold">
              {streak.currentStreakWeeks} <span className="text-xl font-bold">wk</span>
            </p>
          </div>
        ...
        <p className="mt-2 text-sm text-white/90">
          {safe
            ? 'This week is secured. Nice work!'
            : alive
              ? 'Return one container this week to keep your streak alive.'
              : 'Your streak has cooled off — log a return to start a new one.'}
        </p>
      </div>

      {/* Points + weekly cap remaining */}
      <div className="grid grid-cols-2 gap-3">
        <Stat label="Points balance" value={points.toLocaleString()} emoji="⭐" />
        <Stat
          label="Point-earning returns left this week"
          value={`${earningLeft}`}
          emoji="📦"
        />
      </div>
      ...
      <Button onClick={() => setSheetOpen(true)}>
        <span className="text-lg">📲</span> Log a return
      </Button>
      <p className="text-center text-xs text-slate-400">
        Points only come from verified returns — never the honour system.
      </p>
